...
public:
	...
	void Cipher(unsigned char* input, unsigned char* output);  // 译成16位输入密码
	void InvCipher(unsigned char* input, unsigned char* output);  // 破译16位输入
	
	
private:
	int Nb;         // 32位字中的块大小。总是为AES 4。(128位)。
...
public:
	...
	DWORD OnAesUncrypt(LPVOID InBuffer, DWORD InLength, LPVOID OutBuffer);			//AES 解密数
