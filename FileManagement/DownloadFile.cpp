...
DownloadFile::DownloadFile(QString mFileName, QString mFilePath,int num,QWidget *parent) :
	QWidget(parent)
{
	//ui->setupUi(this);
	//StotalSize = 0;
...
//			disconnect(tcpSocket, SIGNAL(readyRead()), tcp, SLOT(readMessages()));  //取消TCP的正常读信息
...
//
//	//暂时取消TCP原本的验证消息
//	disconnect(tcpSocket, SIGNAL(readyRead()), tcp, SLOT(readMessages()));
//	connect(tcpSocket, SIGNAL(readyRead()), this, SLOT(showFileInfo()));
//}
...
//{
//	initModel();
//	QByteArray dataread = tcpSocket->readAll();
//	//QDataStream in(tcpSocket);
//
//	qDebug() << dataread;
//
...
//	//用完后断开
//	disconnect(tcpSocket, SIGNAL(readyRead()), this, SLOT(showFileInfo()));
//	connect(tcpSocket, SIGNAL(readyRead()), tcp, SLOT(readMessages()));
//	
//}
